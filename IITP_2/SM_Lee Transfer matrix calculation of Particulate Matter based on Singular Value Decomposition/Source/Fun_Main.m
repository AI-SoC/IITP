...
%% PM 변환 행렬 계산
% 변환 행렬 F를 구한 후, 장비 PC값에 곱하여 장비 PM값 획득
% 기본 변환행렬 계산식
% F = Result_Total2\DB_PM;
% Result_PM2 = Result_Total2*F;

% float형을 사용할 수 없으므로, 정수형 변환을 위해 2^16을 곱하여 계산하여 
% 변환행렬을 구하고, 변환행렬을 곱한 후 2^16을 다시 나눠주는 방식을 취함

% cut1을 기준으로 저농도, 고농도를 분리하였으므로, 변환행렬을 개별적으로 계산
F1 = floor(Result_Total2(1:cut1,:)\DB_PM(1:cut1,:)*(2^16));
F2 = floor(Result_Total2(cut1+1:end,:)\DB_PM(cut1+1:end,:)*(2^16));
Result_PM21 = floor(Result_Total2(1:cut1,:)*F1/(2^16));
Result_PM22 = floor(Result_Total2(cut1+1:end,:)*F2/(2^16));

save([filename,'_cut.mat'],'cut1');
% 두 변환행렬을 합쳐서 저장
Result_PM2 = [Result_PM21;Result_PM22];
...
%     PM_legend{7} = 'PM 10.0';
    if 1
        figure
        set(gcf,'position',[575   393   690   490]);
        subplot(2,2,1)
        plot(DB1)
        xlabel('Sample')
        ylabel('grimm PC')
    ...
    if 0
        figure
        subplot(3,2,1)
        plot(DB_PM(:,2)-DB_PM(:,1))
        xlabel('Sample')
        ylabel('PM2.5 - PM1.0')
        subplot(3,2,3)
        plot(DB_PM(:,3)-DB_PM(:,2))
        xlabel('Sample')
        ylabel('PM4.0 - PM2.5')
        subplot(3,2,5)
        plot(DB_PM(:,4)-DB_PM(:,3))
        xlabel('Sample')
        ylabel('PM10.0 - PM4.0')
        
        subplot(3,2,2)
        plot(Result_PM2(:,2)-Result_PM2(:,1))
        xlabel('Sample')
        ylabel('PM2.5 - PM1.0')
        subplot(3,2,4)
        plot(Result_PM2(:,3)-Result_PM2(:,2))
        xlabel('Sample')
        ylabel('PM4.0 - PM2.5')
        subplot(3,2,6)
        plot(Result_PM2(:,4)-Result_PM2(:,3))
        xlabel('Sample')
        ylabel('PM10.0 - PM4.0')
    end
end
